...
unsigned			ncpus;			/* cpu核心数量 */

malloc_mutex_t		arenas_lock;
arena_t				**arenas;		/* 全局存储所有的 arena */
...
choose_arena_hard(void)
{
	...
	if (narenas_auto > 1) {
		...
		for (i = 1; i < narenas_auto; i++) {
			if (arenas[i] != NULL) {
				...
				// 循环遍历所有arenas, 找到绑定thread数量最小的arena, 并记录
				// first_null索引值
		...
		if (arenas[choose]->nthreads == 0
			...
			// 若选定的arena绑定thread为0, 或者当前arena数组中已满, 则返回
			// 被选中的arena
		...
		// 若不存在多于一个arena(单核cpu或人为强制设定), 则返回唯一的
		// 0号arena
		ret = arenas[0];
		malloc_mutex_lock(&arenas_lock);
		ret->nthreads++;
	...
	// TLS全称为Thread Local Storage, 即线程本地存储
	// TSD全称为Thread Specific Data, 即线程私有数据

	// 将已绑定的arena设置到tsd中 (pthread_setspecific)
	arenas_tsd_set(&ret);
...
malloc_init_hard(void)
{
	...
	/* 
	 * base是jemalloc内部用于meta data分配的保留区域, 使用内部独立的分配方式. 
	 ...
	 * 2) chunk_dss_boot, chunk dss指chunk分配的dss(Data Storage Segment)方式. 
	 *    其中涉及dss_base, dss_prev指针的初始化工作.
	...
	/*
	 * 主要是确认arena_maxclass, 这个size代表arena管理的最大region, 
	 * 超过该值被认为huge region. 先通过多次迭代计算出map_bias, 再用
	 * chunksize - (map_bias << LG_PAGE) 即可得到.
	 * 另外还对另一个重要的静态数组arena_bin_info执行了初始化.
	 */
	arena_boot();
