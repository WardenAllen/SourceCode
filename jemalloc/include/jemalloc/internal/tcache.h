...
struct tcache_bin_s 
{
	...
	/* 
	 * 记录两次gc间tcache内部使用的最低水线. 
	 * 该数值与下一次gc时尝试释放的region数量有关. 
	 * 释放量相当于low water数值的3/4. 
	 */
	int low_water;

	/* Fill (ncached_max >> lg_fill_div). */
	/*
	 * 用作tcache refill时作为除数. 当tcache耗尽时, 会请求arena run进行refill. 
	 * 但refill不会一次性灌满tcache, 而是依照其最大容量缩小2^lg_fill_div的倍数. 
	 * 该数值同low_water一样是动态的, 两者互相配合确保tcache处于一个合理的充满度.
	 */
	unsigned lg_fill_div;

	/* # of cached objects. */
	/* 指当前缓存的region数量, 同时也代表栈顶index. */
	unsigned ncached;

	/* Stack of available objects. */
	/* 保存region指针的stack, 称为avail-stack. */
	void **avail;
};

/* 
 * TLS / TSD是另一种针对多线程优化使用的分配技术, jemalloc中称为tcache.
 * tcache解决的是同一cpu core下不同线程对heap的竞争.
 * 通过为每个线程指定专属分配区域, 来减小线程间的干扰.
 * 但显然这种方法会增大整体内存消耗量. 为了减小副作用, jemalloc将tcache
 * 设计成一个bookkeeping结构, 在tcache中保存的仅仅是指向外部region的指针, 
 * region对象仍然位于各个run当中. 换句话说, 如果一个region被tcache记录了, 
 * 那么从run的角度看, 它就已经被分配了.
 */
...
struct tcache_s 
{
	...
	/* 链接节点, 用于将同一个arena下的所有tcache链接起来. */
	...
	/* Event count since incremental GC (Garbage Collection). */
	/*
	 * tcache内部的一个周期计数器. 
	 * 每当tcache执行一次分配或释放时, ev_cnt会记录一次.
	 * 直到周期到来, jemalloc会执行一次incremental gc.
	 * 这里的gc会清理tcache中多余的region, 将它们释放掉. 
	 * 尽管这不意味着系统内存会获得释放, 但可以解放更多的region交给其他更饥饿的线程以分配.
	 */
	unsigned ev_cnt;

	/* Next bin to GC. */
	/* 指向下一次gc的binidx. tcache gc按照一周期清理一个bin执行. */
