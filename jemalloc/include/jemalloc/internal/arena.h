...
struct arena_chunk_s 
{
	...
	/* 
	 * 用于arena chunks_dirty（rb tree）的链接节点.
	 * 如果某个chunk内部含有任何dirty page, 就会被挂载到arena中的chunks_dirty tree上.
	 */
	...
	/* 
	 * available runs又分为dirty和clean两种, 相邻的两种run是无法合并的, 
	 * 除非其中的dirty runs通过purge才可以.该数值记录的就是可以通过purge合并的run数量.
	 */
	size_t nruns_adjac;
	...
	/* 
	 * 动态数组, 每一项对应chunk中的一个page状态(不包含header即map本身的占用).
	 * chunk(包括内部的run)都是由page组成的.page又分为unallocated, small, large三种.
	 ...
	 */
	 ...
	 /* Dynamically sized. 可在后面继续添加arena_chunk_map_t. 所以必须在结尾 */
 ...
 * |  page  |  page  |  page  |  page  |
 * +-----------------------------------------------------------------------------+
 * |  chunk_header  |	 chunk_map	   |  page #0  |  page #1  | ... |  page #n  | 	
 * |	... ...		| [0] [1] ... [n]  |		   |		   |	 |			 |
 * +-------------------|---|-------|---------------------------------------------+
 *					   |   |	   |		 ^			 ^				   ^
 *					   +---|-------|---------+			 |				   |
...
/* 
 * run才是真正负责分配的主体(前提是对small region来说).
 * run的大小对齐到page size上, 并且在内部划分成大小相同的region.
 * 当有外部分配请求时, run就会从内部挑选一个free region返回.
 * 可以认为run就是small region仓库. 
 * 
 * run的大小就是其内部region的大小。
 * 假设run大小为4KB, 则可以把run看成是4KB的region的仓库, 
 * 真正分配给user memory的是其内部的region。
 */
struct arena_run_s 
{
...
/* 
 * 正如chunk通过chunk map记录内部所有page状态一样, run通过在header后挂载bitmap来记录其
 * 内部的region状态. bitmap之后是regions区域. 内部region大小相等, 且在前后都有redzone
 * 保护(需要在设置里打开redzone选项). 
 */
...
struct arena_bin_info_s 
{
	...
	/* 与当前bin的size class相关联的region size. */
	size_t reg_size;
	...
	/* 当前bin的size class相关联的run中region数量. */
	uint32_t nregs;
	...
	/* 当前bin的size class相关联的run中bitmap偏移. */
...
struct arena_bin_s 
{
	...
	/* 
	 * 该lock同arena内部的lock不同, 主要负责保护current run.
	 * 而对于run本身的分配和释放还是需要依赖arena lock.
	 * 通常情况下, 获得bin lock的前提是获得arena lock, 但反之不成立.
	 */
	malloc_mutex_t lock;
	...
	/* 
	 * 当前可用于分配的run, 一般情况下指向地址最低的non-full run, 
	 * 同一时间一个bin只有一个current run用于分配. 
	 */
	arena_run_t *runcur;
	...
	/*
	 * rb tree, 记录当前arena中该bin对应size class的所有non-full runs. 
	 * 因为分配是通过current run完成的, 所以也相当于current run的仓库.
	 */
...
struct arena_s 
{
	...
	/*
	 * 局部arena lock, 取代传统分配器的global lock.
	...
	/* ring queue, 注册所有绑定线程的tcache, 作为统计用途, 需要打开统计功能. */
	ql_head(tcache_t) tcache_ql;

	uint64_t prof_accumbytes;

	/* 代表当前chunk alloc时对系统内存的使用策略, 分为几种情况 */
	dss_prec_t dss_prec;

	/* Tree of dirty-page-containing chunks this arena manages. */
	/* rb tree（红黑树）, 代表所有包含dirty page的chunk集合. */
	...
	/* 
	 * 是一个缓存变量, 记录最近一次被释放的chunk.
	 * 当arena收到一个新的chunk alloc请求时, 会优先从spare中开始查找, 
	 * 因此频繁分配释放时, 可能导致内部chunk利用率下降的情况.
	 */
	arena_chunk_t *spare;
	...
	/*
	 *rb tree, 记录所有未被分配的runs, 用来在分配new run时寻找合适的available run.
	 * 一般作为alloc run时的仓库.
	 */
